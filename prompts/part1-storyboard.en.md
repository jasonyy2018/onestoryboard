...
## 12-Panel Beat Structure (derived from EL.CINE lock text below)

Decompose the ~15s action described in the EL.CINE lock text into **12 sequential beat panels**:
...
Every panel's content must come strictly from the EL.CINE lock text; do not invent beats not implied by the text.
...
EL.CINE lock text (decompose into the 12 sequential panels above — do not treat as footer-only):
