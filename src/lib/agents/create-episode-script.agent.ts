...
CRITICAL: Return ONLY valid JSON. Example:
{
  "episode": 1,
  "scenes": [
    {
...
export async function createEpisodeScript(
  ...
  // Auto-fill missing sceneId if LLM omitted it
  if (result.scenes.some((s) => !s.sceneId)) {
    result = {
      ...result,
      scenes: result.scenes.map((s, i) => ({
        ...s,
        sceneId: s.sceneId || `E${String(episodeNumber)}S${String(i + 1).padStart(2, "0")}`,
      })),
    };
  }
