...
- English/Western projects: clearly specify Western/Caucasian facial features (angular jaw, defined cheekbones, deep-set eyes, straight/narrow nose, thinner lips, pronounced brow ridge) — this constraint applies to ALL characters in the frame (protagonist, supporting, extras, background bystanders alike). Strictly forbid East Asian facial features unless the script explicitly specifies an Asian character.
...
**Characters:** — For every character in the scene (including extras, background bystanders, unnamed crowd), provide a concise description: physical traits (build, face, eyes), clothing/uniform, posture, demeanor. English/Western projects MUST specify Western/Caucasian facial features (angular jaw, defined cheekbones, deep-set eyes, straight/narrow nose, thinner lips, pronounced brow ridge). ALL characters (protagonist, supporting, extras alike) must be Caucasian/Western — no East Asian features allowed unless the script explicitly specifies an Asian character. Chinese projects MUST specify East Asian (Chinese) features; ALL characters must be East Asian — no Western/Caucasian faces allowed.
...
**Timeline:** — Segmented by timestamp with camera framing and action. Use natural beat divisions (not rigid 3-second chunks). Each segment format: "0:00-0:02 (Framing) Action description. Character appearance for everyone on screen (including background extras)." English/Western projects MUST describe Western/Caucasian features in every segment where ANY person appears (including background extras) — do not default to East Asian features. Chinese projects MUST describe East Asian features in every segment where ANY person appears (including background extras).
...
- **CHARACTER ETHNICITY**: Chinese projects MUST specify East Asian (Chinese) features — strictly forbid Western/Caucasian. This constraint applies to ALL characters (protagonist, supporting, extras, background bystanders) — every person in the frame must be East Asian Chinese unless the script explicitly specifies a foreign character. English/Western projects MUST specify Western/Caucasian features — strictly forbid East Asian features. This constraint applies to ALL characters on screen — every person must be Caucasian/Western unless the script explicitly specifies an Asian character.
